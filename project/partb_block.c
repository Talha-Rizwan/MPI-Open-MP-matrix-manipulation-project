...
{
//Matrix a and b will be the inputs 
//Result will be stored in matrix c
int ok=0;
...
// Each message's tag is 1
    for (dest=1; dest <= slaveTaskCount; dest++)
    {
      ...
      MPI_Send(&for_matrix_a[offset][0], rows*col_matrix_a, MPI_DOUBLE,dest,1, MPI_COMM_WORLD);
      // Matrix B is sent
      MPI_Send(&for_matrix_b, col_matrix_a*col_matrix_b, MPI_DOUBLE, dest, 1, MPI_COMM_WORLD);
...
// Root process waits untill the each slave proces sent their calculated result with message tag 2
    for (int i = 1; i <= slaveTaskCount; i++)
    {
      ...
      MPI_Recv(&for_matrix_c[offset][0], rows*col_matrix_b, MPI_DOUBLE, source, 2, MPI_COMM_WORLD, &status);
    }
...
// Slave Processes 
  if (processId > 0) {
    ...
    MPI_Recv(&for_matrix_a, rows*col_matrix_a, MPI_DOUBLE, source, 1, MPI_COMM_WORLD, &status);
   // The slave process receives the Matrix B
	//cout<<"Hell"<<rows_matrix_b<<"  "<<col_matrix_b<<endl;
    MPI_Recv(&for_matrix_b, col_matrix_a*col_matrix_b, MPI_DOUBLE, source, 1, MPI_COMM_WORLD, &status);
    ...
    MPI_Send(&for_matrix_c, rows*col_matrix_b, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD);
  }
}
int main(int argc, char **argv)
{
	...
	//For calculating the matrix a , matrix b rows and columns according to the optimal order
...
// MPI environment is initialized
  MPI_Init(&argc, &argv);
//matrix multiplication will multiply the matrices
